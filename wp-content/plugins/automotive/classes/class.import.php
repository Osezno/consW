...
if(!class_exists("Auto_Import")){
    class Auto_Import {
	    ...
	    public function get_listing_meta(&$listing_categories, &$listing_categories_safe, $dependancy_categories, $row, $csv){
		    global $Listing, $lwp_options, $awp_options;
		    ...
		    $post_options = array(
			    ...
			    "custom_badge"      => $this->search_array_keys($row, "listing_badge", $csv),
			    "custom_tax_inside" => $this->search_array_keys($row, "tax_label_listing", $csv),
			    "custom_tax_page"   => $this->search_array_keys($row, "tax_label_inventory", $csv),
			    "price" => array(
		    ...
		    if($Listing->is_yoast_active()){
			    // meta desc
			    $desc = $this->search_array_keys($row, "_yoast_wpseo_metadesc", $csv);
			    $post_meta["_yoast_wpseo_metadesc"] = (string)esc_html($desc);

			    // focus keyword
			    $keyword = $this->search_array_keys($row, "_yoast_wpseo_focuskw", $csv);
			    ...
			    if(isset($_POST['yoast_title_from_values']) && !empty($_POST['yoast_title_from_values'])) {
				    $title = $this->get_multiple_values( $_POST['yoast_title_from_values'], $row, $csv );
        ...
        public function insert_listings($rows, $additional_options = array()){
            ...
            if(isset($_POST['csv']) && !empty($_POST['csv'])){
                ...
                if(!empty($csv)){
                    ...
                    $current_listings = get_posts( array( "post_type" => "listings", "posts_per_page" => -1, 'post_status' => 'any' ) );
                    ...
                    foreach($rows as $key => $row){
	                    ...
	                    if( (isset($current_check) && ((isset($search_value) && !in_array($search_value, $current_check) )) || (isset($no_check) && $no_check))){
	                        ...
	                        if(!$this->is_wpml_active()){
		                        ...
		                        $insert_id = $this->automotive_wpml_insert_post( $insert_info );
	                        }
                            ...
                            if(!empty($duplicate_ids)){
                                ...
                                foreach($duplicate_ids as $duplicate_id){
                                    ...
                                    if(!empty($update_post) && isset($_POST['overwrite_existing']) && $_POST['overwrite_existing'] == "on"){
	                                    ...
	                                    if(!empty($get_options['custom_badge'])){
		                                    $post_options['custom_badge'] = $get_options['custom_badge'];
	                                    }
