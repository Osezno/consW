...
to retrieve which listing the form was sent from.',
				...
				array(
					...
					'fields' => array(
						/*array(
							'id'        => 'section-start-rev',
							'type'      => 'section',
							'title'     => __('Styling Options', 'listings'),
							'subtitle'  => __('With the "section" field you can create indent option sections.', 'listings'),
							'indent'    => true,
							'class'     => 'revolution_slider_shortcode_section'
						),
						array(
							'title'         => __( 'Element Width', 'listings' ),
							'desc'          => __( 'The width of the element inside the preview area (in pixels).', 'listings' ),
							'id'            => 'revolution_shortcode_width',
							'type'          => 'slider',
							'default'       => '10',
							'min'           => 0,
							'max'           => 600,
						...
						array(
							'id'       => 'revolution_shortcode_background',
							'type'     => 'color',
							'title'    => __('Background Color', 'listings'),
							'subtitle' => __('Pick a background color for the theme (default: #fff).', 'listings'),
							'default'  => '#FFFFFF',
							'validate' => 'color',
						),
						array(
							'id'       => 'revolution_shortcode_font',
							'type'     => 'color',
							'title'    => __('Font Color', 'listings'),
							'subtitle' => __('Pick a background color for the theme (default: #fff).', 'listings'),
							'default'  => '#FFFFFF',
							'validate' => 'color',
						),
			...
			if ( $Listing->is_wpml_active() ) {
				...
				if ( ! empty( $all_languages ) ) {
					// recent vehicle category
					unset( $sections[5]['fields'][23] );

					foreach ( $all_languages as $lang_code => $lang ) {
						array_splice( $sections[5]['fields'], 23, 0, array(
							array(
								'title'    => __( 'Related Vehicle Category', 'listings' ) . ' ' . $lang['display_name'],
								'desc'     => __( 'Use this category to select related vehicles.', 'listings' ),
								'type'     => 'select',
								'id'       => 'related_category_' . $lang_code,
								'required' => array( 'recent_related_vehicles', '!=', 1 ),
								'options'  => $Listing->get_listing_categories_to_redux_select($lang_code),
							)
						) );
					}
