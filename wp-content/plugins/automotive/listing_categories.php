<?php
// todo: add microdata option for values: https://schema.org/Vehicle
...
function listing_categories_page() {
	...
	add_submenu_page( 'edit.php?post_type=listings', __( "Listing Categories", "listings" ), __( "Listing Categories", "listings" ), (isset($lwp_options['listing-category-permission']) && !empty($lwp_options['listing-category-permission']) ? $lwp_options['listing-category-permission'] : "manage_options"), 'listing_categories', 'listing_categories' );
}
...
function listing_categories() {
	...
	<div class="wrap nosubsub">
		...
		<?php if(defined("ICL_LANGUAGE_CODE")){
			$Listing->automotive_message( __( "Using WPML?", "listings" ), __( "We've detected you have WPML active. If you are wanting to make these listing categories multilingual you can do so on this page, simply change the language using the top admin bar and enter in the new language categories.", "listings" ), "info", "wpml_on_site" );
		} ?>
		...
		<div id="col-container">
			...
			<div id="col-right">
				...
				<div class="col-wrap">
					<?php $Listing->automotive_message(__("Looking to change the sort by?", "listings"), __("We've moved the sort by functionality under <b>Listing Options >> Inventory Page >> Sort By Categories</b> and have added the ability to sort by title, date and even random!", "listings"), "info", "sortby_notice"); ?>
									...
									foreach ( $listing_categories as $key => $category ) {
										...
										if ( ! empty( $category['singular'] ) && ! empty( $category['plural'] ) ) {
											...
											<tr id="tag-<?php echo $i; ?>" class="<?php echo( $i & 1 ? " " : "" ); ?>">
												...
												<td class=""><a
														href="<?php echo esc_url( add_query_arg( "edit", $slug ) ); ?>"><?php _e( "Edit", "listings" ); ?></a>
												</td>
												<td class=""><a
														href="<?php echo esc_url( add_query_arg( "delete", $slug ) ); ?>"><?php _e( "Delete", "listings" ); ?></a>
												</td>
				...
				<div class="col-wrap">
					...
					<div class="form-wrap">
						...
						<form id="add_category" method="post" action="" class="validate">
							...
							<div class="form-field">
								<label for="is_number"
								       style="display: inline-block;"><?php _e( "Is Number", "listings" ); ?></label>
								<input name="is_number" id="is_number" type="checkbox" value="1"
								       style="width: auto;"<?php echo( $is_editing && $is_number == 1 ? " checked='checked'" : "" ); ?>>
