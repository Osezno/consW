(function ($) {
    ...
    jQuery(document).ready(function ($) {
        ...
        function reset_rows() {
            ...
            $("#gallery_images .single-gallery-image").each(function (index) {
                var value = (index+1);

                $(this).data("id", value);
                $(this).attr("data-id", value);
                $(this).find(".top_header").text($gallery_images.data("image") + " #" + value);
            });
        }

        if($("#rev_preview_options").length){
            $("#rev_preview_options").tabs();
        }
            ...
            open: function(ev, uii){
                ...
                $( ".ui-multiselect-checkboxes" ).sortable({
                    stop: function (event, ui) {
                        ...
                        var is_selected = $(ev.target).find("option[value='" + value + "']").prop("selected");

                        $(ev.target).find("option[value='" + value + "']").remove();
                        $(ev.target).find("option:eq(" + (index) + ")").before("<option value='" + value + "'" + (is_selected ? " selected='selected'" : "") + ">" + value + "</option>");
                    }
                });
            }
        });
        ...
        $("#gallery_images").on("click", ".make_default_image", function (e) {
            ...
            var the_row = $(this).closest(".single-gallery-image");
        ...
        $(document).on("click", ".delete_image", function (e) {
            ...
            handle.fadeOut(300, function () {
                if($gallery_images.find(".single-gallery-image").length == 1){
                    $gallery_images.append("<div class='no_images'>" + $gallery_images.data("no-images") + "</div>")
                }
        ...
        $(document).on('click.mojoOpenMediaManager', '.add_image_gallery', function (e) {
            ...
            $("#gallery_images").data("current_id", formlabel);
            ...
            media_frame[0].on('select', function () {
                ...
                var html = single_gallery_image_template(id, media_attachment[0], false);

                $("#gallery_images .single-gallery-image[data-id='" + id + "']").html(html);
        ...
        $(document).on("click", ".gallery-image-view li", function(e){
            ...
            if($(this).data("layout") == "boxed_fullwidth"){
                $gallery_images.addClass("boxed");

                $.cookie('gallery_layout', 'boxed');
            } else {
                $gallery_images.removeClass("boxed");

                $.cookie('gallery_layout', 'full');
            }

            // fix for sortable style being added after drag
            $gallery_images.find(".single-gallery-image").each( function(){
        ...
        function single_gallery_image_template(id, val, container){
            var html = "";

            var $gallery_images     = $("#gallery_images");

            var image_text          = $gallery_images.data("image");
            var change_image_text   = $gallery_images.data("change-image");
            var set_default_text    = $gallery_images.data("set-default");
            var delete_image_text   = $gallery_images.data("delete-image");
            ...
            html += "<div class='image_preview' data-alt='" + val.alt + "' data-full-image='" + val.sizes.full.url + "'><img src='" + size.url + "'></div>";
            html += "<div class='buttons'><span class='button add_image_gallery' data-id='" + id + "'><span>" + change_image_text + "</span> <i class=\"fa fa-pencil\"></i></span> ";
            html += "<span class='button make_default_image'><span>" + set_default_text + "</span> <i class=\"fa fa-hand-pointer-o\"></i></span> ";
            html += "<span class='button delete_image'><span>" + delete_image_text + "</span> <i class=\"fa fa-trash\"></i></span> ";
        ...
        $(document.body).on('click.mojoOpenMediaManager', '.add_image', function (e) {
            ...
            $gallery_images.find(".no_images").remove();

            var id = ($gallery_images.find(".single-gallery-image").length + 1);
            ...
            media_frame[3].on('select', function () {
                ...
                $.each(media_attachment, function (i, val) {
                    var html = single_gallery_image_template(id, val, true);
        ...
        function single_gallery_url_image_template(){
            var id                  = ($("#gallery_images .single-gallery-image").length + 1);
            var delete_image_text   = $gallery_images.data("delete-image");
            var url_text            = $gallery_images.data("url");

            var html = "";

            html += "<div class='single-gallery-image' data-id='"+id+"'><div class='top_header'><input type='url' name='gallery_images[]' placeholder='" + url_text + "'></div>";
            html += "<div class='image_preview'><img></div>";
            html += "<div class='buttons'><span class='button delete_image'><span>" + delete_image_text + "</span> <i class='fa fa-trash'></i></span></div> ";
        ...
        $(document).on("click", ".add_image_hotlink", function(e){
            ...
            var html = single_gallery_url_image_template();

            $("#gallery_images").append(html);
        });

        $("#gallery_images.hotlink img").error( function(){
        ...
        $(document).on("keyup input paste", "#gallery_images.hotlink .top_header input", function(){
            ...
            $(this).closest(".single-gallery-image").find(".image_preview img").attr("src", image_url);
        });
        ...
        $("#listing_video_input").bind("keyup input paste", function () {
            ...
            if (youtubeUrl) {
                var output = '<br><br><iframe src="http://www.youtube.com/embed/' + youtubeUrl[1] + '?rel=0" height="400" width="644" allowfullscreen="" style="display: none;" class="listing-video-preview"></iframe>';
                $("#listing_video").html(output);
                $(".listing-video-preview").slideDown();
            } else if (vimeoUrl) {
                var output = '<br><br><iframe src="http://player.vimeo.com/video/' + vimeoUrl[3] + '" height="400" width="644" allowfullscreen="" style="display: none;" class="listing-video-preview"></iframe>';
                $("#listing_video").html(output);
                $(".listing-video-preview").slideDown();
            } else {
                var output = '<br><br><video height="400" width="644" allowfullscreen="" style="display: none;" class="listing-video-preview" controls="true"><source src="' + url + '"></video>';
                $("#listing_video").html(output);
                $(".listing-video-preview").slideDown();
            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    ...
                    if (type == "options") {
                        ...
                        features_table.find("tr:last").append("<td><label><input type=\"checkbox\" value=\"" + escapeHtml(response.name) + "\" name=\"multi_options[]\" checked=\"checked\">" + escapeHtml(response.name) + "</label></td>");
        ...
        $(document).on("click", ".install_automotive_demo_content", function (e) {
            ...
            var nonce = $(".automotive_one_click_installer_nag").data("nonce");
            ...
            jQuery.ajax({
                ...
                url: myAjax.ajaxurl,
                type: 'POST',
                data: {action: 'automotive_demo_content_installer', step: 1, nonce: nonce},
                success: function (response) {
                    if (response == "success") {
                        ...
                        jQuery.ajax({
                            ...
                            url: myAjax.ajaxurl,
                            type: 'POST',
                            data: {action: 'automotive_demo_content_installer', step: 2, nonce: nonce},
                            success: function (response) {
                                if (response == "success") {
                                    ...
                                    jQuery.ajax({
                                        ...
                                        url: myAjax.ajaxurl,
                                        type: 'POST',
                                        data: {action: 'automotive_demo_content_installer', step: 3, nonce: nonce},
                                        success: function (response) {
                                            if (response == "success") {
                                                ...
                                                jQuery.ajax({
                                                    ...
                                                    url: myAjax.ajaxurl,
                                                    type: 'POST',
                                                    data: {action: 'automotive_demo_content_installer', step: 4, nonce: nonce},
                                                    success: function (response) {
                                                        if (response == "success") {
        ...
        $(document).on("click", ".save_import_categories", function (e) {
            ...
            jQuery.ajax({
                url: myAjax.ajaxurl,
                type: 'POST',
                data: {action: 'save_import_categories', form: $("#csv_import").serialize(), nonce: $("#csv_import").data("nonce") },
                success: function (response) {
                    alert(response);
                }
            });
        ...
        $(document).on("click", ".edit_name_text", function(e){
            ...
            $this_cell.html("<input type='text' class='current_edit_value'> <button class='button-primary save_edited_value' data-original-text='" + escapeHtml(current_value) + "'>" + $(".listing_table").data("save-text") + "</button>");
                    ...
                    jQuery.ajax({
                        ...
                        success: function (response) {
                            if (response.status == "true") {
                                ...
                                // update buttons
                                $this_row.find(".delete_name").data('id', response.slug);
                                $this_row.find(".edit_name_text").data('id', response.slug);
                            } else {
    ...
    $(document).on("change", "#rev_slider_template", function(){
        ...
        $("#rev_shortcode").val("[auto_card id='" + $("#rev_shortcode").data('listing-id') + "' template='" + template_id + "']");
    });
...
(function ($) {
    ...
    ListingYoastPlugin.prototype.getContent = function (content) {
        ...
        if($("#gallery_images .single-gallery-image").length) {
            $("#gallery_images .single-gallery-image").each(function () {
                var $image_preview = $(this).find(".image_preview");

                var alt = $image_preview.data("alt");
                var img = $image_preview.data("full-image");

                extra_content += "<img src='" + img + "' alt='" + alt + "'>";
            });
        }

        return extra_content ? ( extra_content + content ) : content;
    };
