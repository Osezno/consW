	...
	if(!is_wp_error($response)) {
		...
		if ( isset( $body_encode->years[0]->styles ) && ! empty( $body_encode->years[0]->styles ) ) {
			foreach ( $body_encode->years[0]->styles as $year_key => $year_details ) {
				...
				$style_equipment = get_vin_options( $style_id );
...
function get_all_style_ids($info){
	$years     = (isset($info['years'][0]['styles']) && !empty($info['years'][0]['styles']) ? $info['years'][0]['styles'] : "");
	$style_ids = array();

	if(!empty($years)){
		$style_ids = wp_list_pluck($years, "id");
	}
...
//	        die;
            ...
            <?php if(!empty($vin) && !isset($_GET['error'])){
                                            ...
                                            // if is array
                                                ...
                                                foreach($info_value as $deep_key => $deep_value){
                                                    if($key == "options" || $key == "colors"){
                                                        // D($deep_value);
                                                        if(!empty($deep_value)){
                                                            echo "<h5>" . ucwords($key) . " " . __("option set", "listings") . " " . $i . "</h5>\n";
                                                        ...
                                                        echo "<h5>" . ucwords($key) . " " . __("submodel", "listings") . " " . $i . "</h5>\n";
	                                                    ...
	                                                    if(isset($deep_value['equipment']) && !empty($deep_value['equipment'])){
		                                                    ...
		                                                    echo "<h3>" . __("Equipment Options", "listings") . "</h3>";
		                                                    ...
		                                                    foreach($deep_value['equipment'] as $equipment_key => $equipment_option_value){
			                                                    ...
			                                                    if(!empty($equipment_option_value['attributes'])){
				                                                    echo "<h3>" . $equipment_option_value['name']."</h3>";

				                                                    echo "<div>";
				                                                    foreach($equipment_option_value['attributes'] as $equipment_value_id => $equipment_values){
					                                                    echo "<li class='ui-state-default'>name: " . $equipment_values['name'] . " <input type='hidden' name='' value='" . $key . "|" . $i . "|" . $info_key . "|" . $i . "|equipment|" . $equipment_key . "|attributes|" . $equipment_value_id . "|name' /></li>\n";
					                                                    echo "<li class='ui-state-default'>value: " . $equipment_values['value'] . " <input type='hidden' name='' value='" . $key . "|" . $i . "|" . $info_key . "|" . $i . "|equipment|" . $equipment_key . "|attributes|" . $equipment_value_id . "|value' /></li>\n";
				                                                    }
				                                                    echo "</div>";
			                                                    }
		                                                    }
                    ...
                    <form method="post" action="" name="vin_import_form" id="vin_import_form" data-nonce="<?php echo wp_create_nonce("automotive_vin_import"); ?>">
                        <?php
    ...
    jQuery(document).ready( function($){
        ...
        $(document).on("click", ".save_vin_import_categories", function(e){
            ...
            jQuery.ajax({
                url: myAjax.ajaxurl,
                type: 'POST',
                data: { action: 'save_vin_import_categories', form: $("#vin_import_form").serialize(), nonce: $("#vin_import_form").data('nonce') },
