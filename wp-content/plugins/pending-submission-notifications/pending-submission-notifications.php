...
function pending_submission_notifications_send_email( $new_status, $old_status, $post ) {
	...
	if ( 'pending' === $new_status && user_can( $post->post_author, 'edit_posts' ) && ! user_can( $post->post_author, 'publish_posts' ) ) {
		...
		$adminArray = explode(',', $pending_submission_email);
		$admin1 = $adminArray[0];
		$admin2 = $adminArray[1];
		$admin3 = $adminArray[2];
		$admin4 = $adminArray[3];
		...
		$message                 .= __( 'admin1', 'pending-submission-notifications' ) . ': ' . $admin1 . "\r\n";
		$message                 .= __( 'admin2', 'pending-submission-notifications' ) . ': ' . $admin2 . "\r\n";
		...
		$message                 .= __( 'category', 'pending-submission-notifications' ) . ': ' . $category . "\r\n";
	...
	elseif ( 'pending' === $old_status && 'publish' === $new_status && user_can( $post->post_author, 'edit_posts' ) && ! user_can( $post->post_author, 'publish_posts' ) ) {
		...
		$message  = '"' . $post->post_title . '" ' . __( 'was just published a', 'pending-submission-notifications' ) . "! \r\n\r\n";
		$message .= $url;
